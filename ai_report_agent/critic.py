...
# Settings 提供 DeepSeek 配置、调试目录等运行配置。
...
class ReportSection:
    ...
    # 该区块的完整 Markdown 内容，包含标题行本身。
    content: str

    # 该区块在报告中的顺序。__metadata__ 使用 0，二级标题区块从 1 开始。
    index: int

"""
`field()` 是 `dataclasses` 里的一个函数，通常配合 `@dataclass` 使用，用来给某个字段做更细的配置。

最常见用途：**给列表、字典这种可变对象设置默认值**。
...
因为列表是“可变对象”。如果直接这样写，多个对象可能会共享同一个列表，容易出 bug。
...
> 每次创建 `RevisionPlan` 对象时，都重新调用 `list()` 生成一个新的空列表。
...
a1.items.append("hello")

print(a1.items)  # ['hello']
print(a2.items)  # []
```
...
在 dataclass 里这通常会直接报错，因为 Python 不希望你这样写可变默认值。
...
class RevisionPlan:
    ...
    # 全局修订要求，例如修正日期、删除无来源判断。
    global_instructions: list[str] = field(default_factory=list)

    # 每个区块对应的具体修订要求：区块标题 -> 指令列表。
    section_instructions: dict[str, list[str]] = field(default_factory=dict)
    ...
    def valid(self) -> bool:
        ...
        # 计划有效必须同时满足：没有拒绝原因，并且至少指定了一个要修订的区块。
...
class RevisionWorkflowResult:
    ...
    # 最终可使用的 Markdown。可能是修订后的报告，也可能是保留的原报告。
    markdown: str
    ...
    # 成功被 LLM 修订并通过本地校验的区块标题。
    revised_sections: list[str] = field(default_factory=list)

    # LLM 生成了修订但被本地校验拒绝的区块说明。
    rejected_sections: list[str] = field(default_factory=list)


def compact_report_for_critic(report: str, max_reference_lines: int = 12) -> str:
    """压缩给 LLM critic 的日报文本，降低 token 和内容风控风险。

    critic 的核心任务是检查正文质量、日期和重复问题，不需要读取几十条完整链接。
    因此这里保留主体区块，压缩“参考来源”，并保留“采集状态”摘要。
    原始完整报告仍然会用于最终保存，不会被这个函数修改。
    """

    # 先按二级标题拆分报告，这样可以单独处理“参考来源”“采集状态”。
    sections = split_report_sections(report)

    # compact_sections 保存压缩后的区块，最后再拼回 Markdown。
    compact_sections: list[ReportSection] = []

    for section in sections:
        # 标题归一化后更容易匹配，例如去掉空格、大小写和符号差异。
        normalized_title = normalize_section_title(section.title)

        # 参考来源通常最长，且包含大量 URL；critic 不需要完整读取每一条。
        if normalized_title == "参考来源":
            compact_sections.append(
                ReportSection(
                    title=section.title,
                    content=compact_reference_section(section.content, max_reference_lines),
                    index=section.index,
                )
            )
            continue

        # 采集状态可能包含很长的错误列表，也做长度保护。
        if normalized_title == "采集状态":
            compact_sections.append(
                ReportSection(
                    title=section.title,
                    content=compact_collect_status_section(section.content),
                    index=section.index,
                )
            )
            continue

        # 其他主体内容原样保留，保证 critic 仍能检查正文质量。
        compact_sections.append(section)

    # 把压缩后的区块拼回一个 Markdown 字符串。
    return merge_report_sections(compact_sections)


def compact_reference_section(content: str, max_lines: int) -> str:
    """压缩参考来源区块，只保留前若干条来源标题。"""

    # splitlines 会保留每行文本，方便识别 "- " 开头的来源列表项。
    ...
    # 只统计 Markdown 列表项，忽略空行和其他说明文字。
    reference_lines = [line for line in lines[1:] if line.strip().startswith("- ")]

    # 保留前 max_lines 条，其余只记录省略数量。
    kept_lines = reference_lines[:max_lines]
    skipped_count = max(0, len(reference_lines) - len(kept_lines))

    output_lines = [heading, ""]
    output_lines.extend(kept_lines)
    if skipped_count:
        output_lines.append(f"- 其余 {skipped_count} 条参考来源已省略，仅用于降低自查 prompt 长度。")

    return "\n".join(output_lines).strip()


def compact_collect_status_section(content: str, max_chars: int = 800) -> str:
    """压缩采集状态区块，避免长错误列表进入 critic prompt。"""

    # 未超过限制时原样返回。
    if len(content) <= max_chars:
        return content

    # 超过限制时截断，并留下明确说明，避免让 critic 误以为原文就是半截。
    return content[:max_chars].rstrip() + "\n\n...采集状态过长，后续内容已省略。"


def format_critic_fallback(error: Exception, local_critique: str) -> str:
    """把 LLM critic 失败格式化成可被主流程继续处理的结果。

    如果本地硬性检查已经发现错误，则继续返回 FAIL，让后续修订流程处理。
    如果只是 LLM 自查接口失败，则返回 PASS，避免因为第三方风控或网络抖动导致
    已经生成好的日报无法保存。
    """

    # 如果本地硬检查已经失败，不能因为 LLM critic 失败就放过硬错误。
    if local_critique:
        return f"""{local_critique}
LLM 检查结果：
- LLM 自查未完成：{type(error).__name__}: {error}
"""

    # 没有本地硬错误时，返回“结论：PASS”，让主流程继续保存报告。
    # 注意这里避免出现“日期错误”这类会被 should_revise 误判的词。
    return f"""结论：PASS
主要问题：
- LLM 自查未完成：{type(error).__name__}: {error}
修改建议：
- 本次已完成本地日期/时间硬性检查，未发现需要本地修复的日期/时间问题。
- 由于 LLM critic 接口失败，本次跳过自动修订，保留原始日报。
"""
...
def build_critic_prompt(report: str, profile: UserProfile) -> str:
    ...
    # 压缩参考来源和采集状态，避免把超长链接列表完整送入 critic。
    compact_report = compact_report_for_critic(report)
...
def build_revision_plan_prompt(report: str, critique: str, profile: UserProfile) -> str:
    ...
    # 当前日期继续传给模型，防止修订计划忽略日期约束。
    today = date.today().strftime("%Y-%m-%d")

    # 只允许模型选择报告中真实存在的二级标题，避免它要求修不存在的区块。
...
def build_section_revision_prompt(
    ...
    # 单区块修订也需要知道当天日期，避免局部修订时引入错误时间表达。
    today = date.today().strftime("%Y-%m-%d")
...
def split_report_sections(report: str) -> list[ReportSection]:
    ...
    # 每个二级标题到下一个二级标题之间，视为一个 ReportSection。
...
def parse_revision_plan_response(response: str, report: str) -> RevisionPlan:
    ...
    # LLM 可能包裹 Markdown 代码块或附带解释，先尝试提取 JSON 对象。
    payload = extract_json_object(response)
    if not payload:
        return RevisionPlan(rejected_reason="修订计划不是可解析 JSON。")

    # 建立“归一化标题 -> 原始标题”的映射，用于容错匹配 section_title。
    sections = split_report_sections(report)
    valid_titles = {
    ...
    # global_instructions 可以存在，但不能单独构成可执行 LLM 区块修订。
...
def extract_json_object(text: str) -> dict[str, object]:
    ...
    # 提取第一个 { 到最后一个 } 之间的内容，兼容前后夹杂少量解释文字。
    start = stripped.find("{")
    end = stripped.rfind("}")
...
def critique_report(settings: Settings, report: str, profile: UserProfile) -> str:
    ...
    # 如果第三方接口因为内容风控、网络或临时错误失败，不让整次日报失败。
    try:
        llm_critique = call_deepseek(settings, prompt, "critic_prompt")
    except Exception as exc:
        return format_critic_fallback(exc, local_critique)
...
def revise_report(settings: Settings, report: str, critique: str, profile: UserProfile) -> str:
    ...
    # 保留旧接口：外部只需要 Markdown 时，可以直接调用这个函数。
...
def revise_report_with_plan(
    ...
    # 保留原报告引用，便于理解流程；实际修订从修正元信息后的版本开始。
    original_report = report

    # 先用本地确定性规则修正标题日期和生成时间，减少 LLM 需要处理的硬错误。
    repaired_report = repair_report_metadata(report)

    # 第一步：让 LLM 只输出结构化修订计划。
...
def safe_debug_name(value: str) -> str:
    ...
    # 只保留字母、数字和中文，其他字符用下划线替换。
    cleaned = re.sub(r"[^a-zA-Z0-9\u4e00-\u9fff]+", "_", value).strip("_")

    # 文件名不宜过长；如果清理后为空，使用 section 兜底。

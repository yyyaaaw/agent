...
def build_batch_prompt(items: list[NewsItem], batch_index: int, batch_count: int) -> str:
    """构造单批资讯提炼 prompt。
...
def build_final_prompt(batch_summaries: list[str]) -> str:
    """构造最终日报 prompt。
...
def extract_message_text(result: dict) -> str:
    ...
    如果有 reasoning_content，这里也会一起显示。
    """
    ...
    # get("reasoning_content", "") 表示：
    # 如果有 reasoning_content 字段就取它，否则返回空字符串。
    reasoning = message.get("reasoning_content", "")
    content = message.get("content", "")

    # 如果 API 返回了推理内容，就把推理摘要和最终回答都拼起来。
    if reasoning:
        return f"## 模型推理摘要\n\n{reasoning.strip()}\n\n## 模型回答\n\n{content.strip()}"
...
def analyze_news(settings: Settings, items: list[NewsItem]) -> str:
    ...
    for index, batch in enumerate(batches, start=1):
        ...
        # 构造当前批次的 prompt。
        prompt = build_batch_prompt(batch, index, len(batches))
    ...
    # 构造最终汇总 prompt。
    final_prompt = build_final_prompt(batch_summaries)

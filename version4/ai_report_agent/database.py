...
Version 4 在 Version 3 的 SQLite 长期记忆基础上，加入本地 embedding 向量检索。
...
# embed_text / cosine_similarity 用于本地向量检索版 RAG。
...
def save_raw_news_items(database_path: Path, run_id: str, items: list[NewsItem]) -> None:
    ...
    for item in items:
        ...
        # 把标题、摘要、类别和来源拼成向量文本。
        embedding_text = f"{item.title} {item.summary} {item.category} {item.source}"

        # 生成本地 embedding，并用 JSON 存入 SQLite。
        embedding_json = json.dumps(embed_text(embedding_text))
...
在 agent-v4 中，主要由 retrieve_related_history(...) 实现 RAG 检索：
在调用 DeepSeek 之前，先把本次入选资讯转换成 embedding，

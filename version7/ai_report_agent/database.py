...
Version 6 在 BGE-M3 向量记忆基础上，增加事件表和事件级 RAG。
...
# NewsEvent 是事件聚类后的结构。
from ai_report_agent.events import NewsEvent
    ...
    with connect(database_path) as connection:
        ...
        connection.execute(
            ...
            CREATE TABLE IF NOT EXISTS events (
                ...
                representative_link TEXT NOT NULL DEFAULT '',
        ...
        connection.execute("CREATE INDEX IF NOT EXISTS idx_event_items_event ON event_items(event_id)")
        connection.execute("CREATE INDEX IF NOT EXISTS idx_event_feedback_event ON event_feedback(event_id)")
...
def find_news_item_id(connection: sqlite3.Connection, run_id: str, item: NewsItem) -> int | None:
    """根据 run_id 和去重键查找新闻数据库 ID。"""
    # 生成去重键。
    normalized_link, normalized_title = item_keys(item)
    ...
    row = connection.execute(
        ...
        LIMIT 1
        """,
        (run_id, normalized_link, normalized_title),
    ).fetchone()
...
def save_events(database_path: Path, run_id: str, events: list[NewsEvent]) -> None:
    """保存事件聚类结果。"""
    # 当前写入时间。
    now = utc_now_text()
    ...
    with connect(database_path) as connection:
        for event in events:
            ...
            cursor = connection.execute(
                """
                INSERT INTO events (
                    run_id, event_key, title, summary, sources_json, categories_json,
                    representative_link, average_score, max_score, item_count,
            ...
            # 写入事件和新闻的关联。
            relation_rows = []
            for entry in event.scored_items:
                news_item_id = find_news_item_id(connection, run_id, entry.item)
                if news_item_id is not None:
                    relation_rows.append((event_db_id, news_item_id, now))

            # 批量写入关系表。
            connection.executemany(
                """
                INSERT INTO event_items (event_id, news_item_id, created_at)
                VALUES (?, ?, ?)
                """,
                relation_rows,
            )
...
在 agent-v6 中，主要由 retrieve_related_events(...) 实现事件级 RAG 检索：
在调用 DeepSeek 之前，先用本地 BGE-M3 把本次事件转换成真实语义 embedding，
再从数据库里查找 embedding 相似的历史事件，作为 RAG 上下文输入。
...
现在的知识库就是 SQLite，检索器就是 retrieve_related_events()，生成器就是 DeepSeek。
'''
...
def retrieve_related_events(
    ...
    """从历史数据库中检索与本次事件语义相似的旧事件。"""
    # 没有事件时无法检索。
    if not events:
        return ""

    # 把本次事件合并成查询文本，并生成查询向量。
    query_text = " ".join(f"{event.title} {event.summary}" for event in events)
    ...
    with connect(database_path) as connection:
        rows = connection.execute(
            """
            SELECT title, summary, sources_json, categories_json, average_score,
                   max_score, item_count, embedding_json, created_at
            FROM events
            WHERE run_id != ?
    ...
    # 如果没有足够相似的历史事件，返回空字符串。
    if not scored_rows:
        return ""

    # 格式化为 prompt 上下文。
    lines = ["以下是事件级 RAG 检索找到的相关历史事件，可用于判断趋势延续性："]
    for index, (similarity, row) in enumerate(scored_rows, start=1):
        sources = ", ".join(json.loads(row["sources_json"] or "[]"))
        categories = ", ".join(json.loads(row["categories_json"] or "[]"))
        lines.append(
            "\n".join(
                [
                    f"{index}. {row['title']}",
                    f"   来源：{sources or '未知'}",
                    f"   类别：{categories or '未知'}",
                    f"   新闻数：{row['item_count']}，最高分：{row['max_score']}，相似度：{similarity:.2f}",
                    f"   摘要：{row['summary'][:260]}",
                ]
            )
        )

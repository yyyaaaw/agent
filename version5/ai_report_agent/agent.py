...
# database 模块负责 Version 4 的 SQLite 长期记忆、反馈快照和向量检索。
...
def run_daily_report(settings: Settings) -> Path:
    """运行一次 Version 4 日报 agent，并返回报告路径。"""
    ...
    # 从历史数据库中做向量检索，查找相关旧资讯作为 RAG 上下文。
    ...
    if history_context:
        state.decisions.append("已从 SQLite 数据库完成向量检索，找到相关历史资讯用于最终汇总 prompt。")

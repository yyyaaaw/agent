# 本地 AI 热点日报 Agent - Version 1

这是第一版 agent：固定脚本工作流 + DeepSeek API 分析 + Markdown 日报输出。

当前版本目录：

```text
C:\Users\18352\Desktop\agent\version1
```

这个项目会在本地笔记本上定时收集 AI 热点资讯，调用 DeepSeek API 做中文分析，并生成偏“AI 应用新热点”的 Markdown 汇报。
...
```bash
python version1\catch_ai.py --run-once
```
...
如果使用 Windows 任务计划程序，移动到 `version1` 后建议这样配置：
...
```text
C:\Users\18352\Desktop\agent\version1\reports
```

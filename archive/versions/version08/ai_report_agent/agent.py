...
# database 模块负责 SQLite 长期记忆、事件表和向量检索。
...
# refine_events_with_llm 负责把 embedding 粗聚类事件交给 LLM 二次整合。
...
# format_events_for_state 负责把事件详情写入 runs 状态文件，方便人工查看。
from ai_report_agent.events import cluster_scored_items, format_events_for_state
...
def run_daily_report(settings: Settings) -> Path:
    """运行一次 Version 8 日报 agent，并返回报告路径。"""
    ...
    # 记录本次运行使用了 feedback.json。
    state.decisions.append("已加载 feedback.json 作为本次评分规则。")
    ...
    # 记录 embedding 粗聚类结果。
    state.decisions.append(
        f"事件粗聚类完成：{len(selected_items)} 条入选资讯 -> {len(candidate_events)} 个候选事件"
    )

    # 调用 LLM 对候选事件做二次整合和重新命名，避免一个新闻就是一个事件。
    print("开始调用 DeepSeek 整合事件聚类...")
    events, event_decisions = refine_events_with_llm(settings, candidate_events)

    # 把 LLM 整合过程写入运行轨迹。
    state.decisions.extend(event_decisions)

    # 把最终事件及其相关新闻写入运行轨迹，方便直接从 data/runs/run_state_*.json 查看。
    state.decisions.append("最终事件清单：")
